...
// aprobada: la firma del trabajador y la de RR.HH. van UNA AL LADO DE LA
// OTRA, en la misma fila (trabajador a la izquierda, RR.HH. a la derecha).
// El jefe directo ya no firma acá — cuando de verdad aprobó él (desde su
// perfil), va nombrado en una frase aparte dentro del cuerpo del
// comprobante (ver dibujarCopiaPermiso/dibujarCopiaVacaciones en
// lib/solicitudPdf.js), no como firma física. El timbre institucional va
// debajo de la firma de RR.HH., centrado en el espacio en blanco que queda
// ahí. Devuelve el Y donde termina todo el bloque.
export async function dibujarFirmasAprobadas(
  doc,
  anchoPagina,
  y,
  { trabajadorNombre, segundoFirmante, timbre },
  compacto = false
) {
  ...
  const yLineaSuperior = y + (compacto ? 9 : 16); // espacio en blanco para firmar a mano
  dibujarLineaFirma(doc, centroIzq, yLineaSuperior, anchoLineaFirma, trabajadorNombre, 'Firma del trabajador', compacto);
  const yFinFirmaRRHH = dibujarLineaFirma(
    doc,
    centroDer,
    yLineaSuperior,
    anchoLineaFirma,
  ...
  // Timbre debajo de la firma de RR.HH., centrado en el espacio libre.
  const cyTimbre = yFinFirmaRRHH + (compacto ? 13 : 18);
  const diametroTimbre = compacto ? 22 : 30;
  await dibujarTimbre(doc, timbre, centroDer, cyTimbre, diametroTimbre);

  return cyTimbre + diametroTimbre / 2 + (compacto ? 4 : 6);
}
