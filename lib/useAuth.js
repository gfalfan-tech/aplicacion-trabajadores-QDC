...
export function AuthProvider({ children }) {
  ...
  async function cargarPerfil(userId) {
    ...
    const { data: trabajador } = await supabase
      .from('trabajadores')
      .select('*, jefe_directo:jefe_directo_id(nombre_completo)')
      .eq('id', userId)
      .maybeSingle();
  ...
  // Para refrescar el perfil en el momento (ej. después de subir una foto
  // nueva) sin esperar a que Supabase dispare un evento de auth.
