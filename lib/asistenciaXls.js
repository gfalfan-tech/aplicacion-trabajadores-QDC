...
// La columna "Atraso" de cada fila de día usa formato "H:MM" (ej. "0:02" =
// 2 minutos), a diferencia del total del RESUMEN GENERAL que trae
// "H:MM:SS". Se parsea aparte para no confundir "0:02" con "2 segundos".
function textoHoraMinutoDia(texto) {
  if (texto == null || texto === '') return 0;
  const partes = texto.toString().split(':').map(Number);
  if (partes.length < 2 || partes.some((p) => Number.isNaN(p))) return 0;
  const [h, m] = partes;
  return h * 60 + m;
}
...
// o licencia) y devuelve, por cada día que tuvo atraso y/o que parece
// inasistencia, un detalle {fecha, atraso_minutos, inasistencia}. Los días
// "normales" (sin atraso ni inasistencia) no se incluyen, para mantener el
// detalle compacto. La detección de inasistencia es una aproximación
// (best-effort): días con horario asignado (columna "Asignado"), sin horas
// de asistencia real, y cuyo evento del día no es vacaciones/licencia/libre
// — el número oficial de inasistencias del mes sigue siendo el que trae el
// propio resumen del archivo, esto solo arma el detalle día por día.
function extraerDetalleDias(filas, periodoDesdeISO, periodoHastaISO) {
  ...
  for (let i = 0; i < filas.length; i++) {
    ...
    const atrasoMinutos = textoHoraMinutoDia(fila[10]);

    let esInasistencia = false;
    if (!(eventoEspecial && EVENTOS_NO_INASISTENCIA.has(eventoEspecial))) {
    ...
    if (atrasoMinutos > 0 || esInasistencia) {
      detalle.push({ fecha: fechaISO, atraso_minutos: atrasoMinutos, inasistencia: esInasistencia });
    }
  }
...
function extraerResumenHoja(filas) {
  ...
  const detalleDias = extraerDetalleDias(filas, periodoDesde, periodoHasta);
