...
import { formatearMonto, etiquetaCategoria, etiquetaTipoDocumento, totalLineas } from './rendicionGastosLogica';
...
async function armarPdfRendicionGastos(rendicion) {
  ...
  if (rendicion.estado === 'aprobada' || rendicion.estado === 'rechazada') {
    const ySelloFin = dibujarSelloEstado(doc, rendicion.estado, anchoPagina, y);
    if (rendicion.estado === 'aprobada') {
      dibujarFirmasRendicionGastos(doc, anchoPagina, ySelloFin, {
