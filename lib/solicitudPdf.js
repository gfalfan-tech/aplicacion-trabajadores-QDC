 ...
 * hora_hasta, motivo, estado, tipo_permiso (nombre), aprobado_por (quién
 * resolvió, para saber si fue el jefe directo), y de ser posible
 ...
 * vuelve a ver el PDF). `trabajador` debe traer: nombre_completo, rut,
 * jefe_directo_id y, si está disponible, jefe_directo.nombre_completo —
 * solo se usa para la frase "Autorizado por [nombre] (Jefe Directo)"
 * cuando quien resolvió fue justo el jefe directo asignado; si no, no se
 * firma ni se nombra en el comprobante, solo firman trabajador y RR.HH.
 */
async function dibujarCopiaPermiso(doc, yBase, solicitud, trabajador) {
  ...
  // Solo se nombra al jefe directo si fue justo él quien resolvió la
  // solicitud (no cualquier usuario de RR.HH./administrador) — no firma,
  // solo queda mencionado.
  const jefeAprobador =
    aprobada && trabajador.jefe_directo_id && solicitud.aprobado_por === trabajador.jefe_directo_id
      ? trabajador.jefe_directo?.nombre_completo
      : null;
  if (jefeAprobador) {
    doc.setFont('helvetica', 'italic');
    doc.setFontSize(9);
    doc.text(`Autorizado por ${jefeAprobador} (Jefe Directo).`, MARGEN_IZQ, y);
    y += 6;
  } else {
    y += 3;
  }
  ...
  // disponible. (Cuando está aprobada, ya se dibujó junto con la de RR.HH.
  // arriba y se corta antes de llegar acá — ver el `return` más arriba.)
 ...
 * aprobada, la firma del trabajador junto a la de RR.HH.
 *
 * `solicitud` debe traer: fecha_desde, fecha_hasta, dias_habiles, estado,
 * jefe_nombre (el jefe directo que aprobó en la primera etapa — solo se
 * nombra en una frase, ya no firma), motivo_edicion, fecha_resolucion/
 * created_at (fecha fija del comprobante), y el desglose ya calculado:
 * vacaciones_progresivas, domingo_e_inhabiles, saldo_pendiente.
 */
async function dibujarCopiaVacaciones(doc, yBase, solicitud, trabajador) {
  ...
  const aprobada = solicitud.estado === 'aprobada';
  // jefe_nombre solo viene si el jefe directo de verdad aprobó en la
  // primera etapa (aprobado_por_jefe) — no firma, solo queda mencionado.
  if (aprobada && solicitud.jefe_nombre) {
    doc.setFont('helvetica', 'italic');
    doc.setFontSize(8.5);
    doc.text(`Autorizado por ${solicitud.jefe_nombre} (Jefe Directo).`, MARGEN_IZQ, y);
    y += 5.5;
  } else {
    y += 2;
  }
  ...
  // Respaldo físico: la firma del trabajador va siempre, sin importar el
  // estado en el sistema. (Cuando está aprobada, ya se dibujó junto con la
  // de RR.HH. arriba — ver el `return` más arriba.)
  dibujarFirmaTrabajador(doc, ANCHO_PAGINA, y, trabajador.nombre_completo, true);
}
