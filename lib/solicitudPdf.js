...
async function dibujarCopiaPermiso(doc, yBase, solicitud, trabajador) {
  ...
  y += 4.4;
  doc.setFont('helvetica', 'normal');
  doc.text(`Santiago, ${formatFechaLarga(new Date())}`, MARGEN_IZQ, y);
  y += 9;
  ...
  if (solicitud.estado === 'aprobada' || solicitud.estado === 'rechazada') {
    ...
    if (aprobada) {
      await dibujarTimbre(doc, 'rrhh', ANCHO_PAGINA - MARGEN_DER - 18, yBloqueEmpresa + 3, 17);
    }
  } else {
 ...
 * jefe_nombre, motivo_edicion, y el desglose ya calculado:
