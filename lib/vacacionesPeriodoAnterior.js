...
// Vacaciones del PERÍODO ACTUAL: lo que se lleva devengado desde el
// último aniversario, todavía sin completar el período en curso. Se
// define como el resto del saldo total una vez separado lo "pendiente
// del período anterior" de arriba, así los dos números siempre cuadran
// con el total:
//
//   pendiente_periodo_anterior + periodo_actual = vacaciones_disponibles
export function calcularVacacionesPeriodoActual(saldo) {
  const pendienteAnterior = calcularVacacionesPendientesPeriodoAnterior(saldo);
  const saldoTotal = saldo?.dias_disponibles_estimados || 0;
  return Math.max(0, Math.round((saldoTotal - pendienteAnterior) * 100) / 100);
}
